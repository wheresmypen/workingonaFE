<template>
  <div v-if="visible === true">
    ...
    <div id="permits" class="columns">
        ...
        <div class="column">
        <!-- HERE IS A TOGGLE FOR THE PERMIT BEING APPROVED v-if="showCustomize === true" -->
          <div class="columns">
              ...
              <div class="card">
                <div class="card-header">
                  <p class="card-header-title">
                    {{permit.address}}
                    <!-- <span v-for="type in permit.permitTypes">{{type}} </span> -->
                  </p>
                </div>
                <div class="card-content">
                  {{permit.primaryCompany}}
                  <p>
                  {{permit.description.slice(0,100)}}
                  </p>
                  <p>
                  ${{permit.totalValue}} total value<br/>
                  </p>
                </div>
...
<script>
  ...
  var $ = require('jquery')
  var getPermitsPath = 'http://18.232.157.109:8081/api/client/city?area=boulder&report=master&pageSize='
  var postUserSortInfo = 'http://18.232.157.109:8081/api/client/filters?area=boulder'
